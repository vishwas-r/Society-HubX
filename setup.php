<?php
/**
 * Single-Shot Webapp Setup Wizard for Namma Society
 *
...
function nammasociety_generate_salt() {
    ...
    if (file_exists($config_path)) {
        try {
            ...
            if (defined('ABSPATH')) {
                ...
                // Drop Namma Society custom tables matching Prefix + 'nammasociety51_'
                $nammasociety_tables = $wpdb->get_col("SHOW TABLES LIKE '{$wpdb->prefix}nammasociety51_%'");
                if (is_array($nammasociety_tables)) {
                    foreach ($nammasociety_tables as $table) {
...
if (isset($_POST['action']) && $_POST['action'] === 'run_install') {
    ...
    $site_title = sanitize_input($_POST['site_title'] ?? 'Namma Society Community');
        ...
        if (!$conn) {
            ...
            if ($db_selected) {
                ...
                if (!file_exists($sample_config_path)) {
                    ...
                    $config_content = preg_replace("/define\(\s*'AUTH_KEY',\s*'put your unique phrase here'\s*\);/", "define( 'AUTH_KEY', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'SECURE_AUTH_KEY',\s*'put your unique phrase here'\s*\);/", "define( 'SECURE_AUTH_KEY', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'LOGGED_IN_KEY',\s*'put your unique phrase here'\s*\);/", "define( 'LOGGED_IN_KEY', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'NONCE_KEY',\s*'put your unique phrase here'\s*\);/", "define( 'NONCE_KEY', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'AUTH_SALT',\s*'put your unique phrase here'\s*\);/", "define( 'AUTH_SALT', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'SECURE_AUTH_SALT',\s*'put your unique phrase here'\s*\);/", "define( 'SECURE_AUTH_SALT', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'LOGGED_IN_SALT',\s*'put your unique phrase here'\s*\);/", "define( 'LOGGED_IN_SALT', '" . nammasociety_generate_salt() . "' );", $config_content);
                    $config_content = preg_replace("/define\(\s*'NONCE_SALT',\s*'put your unique phrase here'\s*\);/", "define( 'NONCE_SALT', '" . nammasociety_generate_salt() . "' );", $config_content);
                    ...
                    if (!$written) {
                        ...
                        try {
                            ...
                            if (function_exists('wp_install')) {
                                ...
                                // Activate Namma Society plugin
                                require_once ABSPATH . 'wp-admin/includes/plugin.php';
                                $plugin_path = 'namma-society/namma-society.php';
...
<head>
    ...
    <title>Namma Society — Webapp Setup Wizard</title>
    ...
    <style>
        :root {
            --nammasociety-primary-grad: linear-gradient(135deg, #8b5cf6 0%, #d946ef 100%);
            --nammasociety-bg-grad: radial-gradient(circle at 50% -10%, rgba(139, 92, 246, 0.18) 0%, rgba(217, 70, 239, 0.05) 50%, #05050a 100%);
            --bs-body-bg: #05050a;
        ...
        body {
            background: var(--nammasociety-bg-grad);
        ...
        .nammasociety-setup-container {
            width: 100%;
            max-width: 800px;
        }

        .nammasociety-setup-card {
        ...
        .nammasociety-brand-header {
            text-align: center;
            margin-bottom: 35px;
        }

        .nammasociety-brand-title {
            background: var(--nammasociety-primary-grad);
        ...
        .nammasociety-btn-primary {
            background: var(--nammasociety-primary-grad) !important;
            color: #fff !important;
        ...
        .nammasociety-btn-primary:hover {
            transform: translateY(-2px);
            filter: brightness(1.1);
            box-shadow: 0 8px 25px rgba(139, 92, 246, 0.5) !important;
        }

        .nammasociety-btn-secondary {
        ...
        .nammasociety-btn-secondary:hover {
            background: rgba(255, 255, 255, 0.12) !important;
        }

        .nammasociety-btn-danger {
...
<div class="nammasociety-setup-container">
    <div class="card nammasociety-setup-card border-0">
        
        <!-- Header Section -->
        <div class="nammasociety-brand-header">
            <div class="nammasociety-setup-icon">
                <i class="fa-solid fa-building-shield text-primary display-5"></i>
            </div>
            <h1 class="nammasociety-brand-title">Namma Society</h1>
        ...
        <?php if ($install_success) : ?>
            <div class="text-center py-4">
                ...
                <p class="text-muted px-md-5">WordPress has been installed successfully and the Namma Society management engine is activated. To secure your site, this setup file has self-deleted.</p>
                <div class="d-flex justify-content-center gap-3 mt-4">
                    <a href="<?php echo htmlspecialchars(admin_url('admin.php?page=nammasociety51-setup')); ?>" class="btn nammasociety-btn-primary">
                        <i class="fa-solid fa-building-circle-check me-2"></i>Configure Society Settings
                    </a>
                    <a href="<?php echo htmlspecialchars(admin_url()); ?>" class="btn nammasociety-btn-secondary">
        ...
        <?php elseif ($is_installed) : ?>
            <div class="text-center py-4">
                ...
                <p class="text-muted px-md-5 mb-5">WordPress and the Namma Society plugin are already configured on this server. Running this installer again is blocked to prevent data loss.</p>
                ...
                <div class="card bg-danger bg-opacity-10 border-danger border-opacity-20 p-4 mb-4 rounded-4 text-start">
                    ...
                    <form method="post" action="" onsubmit="return confirm('WARNING: Are you sure you want to delete all database tables and configuration? This action is permanent and cannot be undone!');">
                        ...
                        <button type="submit" class="btn nammasociety-btn-danger px-4 py-2">
                ...
                <div class="d-flex justify-content-center gap-3">
                    <a href="./" class="btn nammasociety-btn-primary">
                        <i class="fa-solid fa-house me-2"></i>Visit Website
                    </a>
                    <a href="./wp-admin/" class="btn nammasociety-btn-secondary">
        ...
        <?php else : ?>
            ...
            <form method="post" action="">
                ...
                <div class="nammasociety-section-title">
                    <i class="fa-solid fa-database text-primary me-2"></i>1. Database Configuration
                </div>
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        <label for="db_host" class="form-label">Database Host</label>
                        <input type="text" name="db_host" id="db_host" class="form-control nammasociety-input" value="<?php echo htmlspecialchars($_POST['db_host'] ?? 'localhost'); ?>" required />
                    </div>
                    <div class="col-md-6">
                        <label for="db_name" class="form-label">Database Name</label>
                        <input type="text" name="db_name" id="db_name" class="form-control nammasociety-input" placeholder="e.g. societydb" value="<?php echo htmlspecialchars($_POST['db_name'] ?? ''); ?>" required />
                    </div>
                    <div class="col-md-6">
                        <label for="db_user" class="form-label">Database Username</label>
                        <input type="text" name="db_user" id="db_user" class="form-control nammasociety-input" placeholder="e.g. root" value="<?php echo htmlspecialchars($_POST['db_user'] ?? ''); ?>" required />
                    </div>
                    <div class="col-md-6">
                        <label for="db_pass" class="form-label">Database Password</label>
                        <input type="password" name="db_pass" id="db_pass" class="form-control nammasociety-input" placeholder="••••••••" />
                    </div>
                    <div class="col-md-6">
                        <label for="db_prefix" class="form-label">Table Prefix</label>
                        <input type="text" name="db_prefix" id="db_prefix" class="form-control nammasociety-input" value="<?php echo htmlspecialchars($_POST['db_prefix'] ?? 'wp_'); ?>" required />
                    </div>
                </div>

                <!-- Section 2: Admin Site Setup -->
                <div class="nammasociety-section-title mt-4">
                    <i class="fa-solid fa-user-gear text-primary me-2"></i>2. Administrator & Site Setup
                </div>
                <div class="row g-3 mb-4">
                    <div class="col-md-12">
                        <label for="site_title" class="form-label">Site Title</label>
                        <input type="text" name="site_title" id="site_title" class="form-control nammasociety-input" value="<?php echo htmlspecialchars($_POST['site_title'] ?? 'Namma Society Community'); ?>" required />
                    </div>
                    <div class="col-md-6">
                        <label for="admin_user" class="form-label">Admin Username</label>
                        <input type="text" name="admin_user" id="admin_user" class="form-control nammasociety-input" value="<?php echo htmlspecialchars($_POST['admin_user'] ?? 'admin'); ?>" required />
                    </div>
                    <div class="col-md-6">
                        <label for="admin_pass" class="form-label">Admin Password</label>
                        <input type="password" name="admin_pass" id="admin_pass" class="form-control nammasociety-input" placeholder="Choose a strong password" required />
                    </div>
                    <div class="col-md-12">
                        <label for="admin_email" class="form-label">Admin Email Address</label>
                        <input type="email" name="admin_email" id="admin_email" class="form-control nammasociety-input" placeholder="admin@yoursite.com" value="<?php echo htmlspecialchars($_POST['admin_email'] ?? ''); ?>" required />
                    </div>
                </div>

                <div class="d-grid mt-4">
                    <button type="submit" class="btn nammasociety-btn-primary btn-lg fs-5 py-3">
