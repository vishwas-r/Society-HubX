...
class SHUBX51_Asset_Manager implements SHUBX51_Module {
	...
	public function __construct() {
		...
		// AJAX Endpoints
		add_action( 'wp_ajax_shubx51_add_asset', array( $this, 'handle_add_asset' ) );
		add_action( 'wp_ajax_shubx51_edit_asset', array( $this, 'handle_edit_asset' ) );
		add_action( 'wp_ajax_shubx51_delete_asset', array( $this, 'handle_delete_asset' ) );
		add_action( 'wp_ajax_shubx51_restore_asset', array( $this, 'handle_restore_asset' ) );
		add_action( 'wp_ajax_shubx51_get_asset', array( $this, 'handle_get_asset' ) );
		...
		// Module Registration
		add_filter( 'shubx51_get_module_assets', array( $this, 'get_instance' ) );
	}
	...
	public function handle_add_asset() {
		if ( wp_doing_ajax() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'shubx51_asset_action' ) && ! wp_verify_nonce( $nonce, 'shubx51_nonce' ) && ! wp_verify_nonce( $nonce, 'shubx51_admin_nonce' ) ) {
	...
	public function handle_delete_asset() {
		if ( wp_doing_ajax() ) {
			$nonce = isset( $_REQUEST['_wpnonce'] ) ? sanitize_key( wp_unslash( $_REQUEST['_wpnonce'] ) ) : '';
			if ( ! wp_verify_nonce( $nonce, 'shubx51_delete_asset_nonce' ) && ! wp_verify_nonce( $nonce, 'shubx51_nonce' ) && ! wp_verify_nonce( $nonce, 'shubx51_admin_nonce' ) ) {
				wp_send_json_error( array( 'message' => 'Nonce verification failed' ), 403 );
			}
		} else {
	...
	public function handle_restore_asset() {
		if ( wp_doing_ajax() ) {
			$nonce = isset( $_REQUEST['_wpnonce'] ) ? sanitize_key( wp_unslash( $_REQUEST['_wpnonce'] ) ) : '';
			if ( ! wp_verify_nonce( $nonce, 'shubx51_restore_asset_nonce' ) && ! wp_verify_nonce( $nonce, 'shubx51_nonce' ) && ! wp_verify_nonce( $nonce, 'shubx51_admin_nonce' ) ) {
				wp_send_json_error( array( 'message' => 'Nonce verification failed' ), 403 );
			}
		} else {
