...
class SGVX51_Rule_Manager implements SGVX51_Module {
	...
	public function __construct() {
		...
		// Admin AJAX Actions
		add_action( 'wp_ajax_sgvx51_add_rule', array( $this, 'handle_add_rule' ) );
		add_action( 'wp_ajax_sgvx51_edit_rule', array( $this, 'handle_edit_rule' ) );
		add_action( 'wp_ajax_sgvx51_delete_rule', array( $this, 'handle_delete_rule' ) );
		...
		add_action( 'wp_ajax_sgvx51_acknowledge_rule', array( $this, 'handle_acknowledge_rule' ) );
		add_action( 'wp_ajax_sgvx51_appeal_violation', array( $this, 'handle_appeal_violation' ) );
		add_action( 'wp_ajax_sgvx51_get_pending_acknowledgments', array( $this, 'handle_get_pending_acknowledgments' ) );
		add_action( 'wp_ajax_sgvx51_search_rules', array( $this, 'handle_search_rules' ) );
		
		// Scheduled Actions
		add_action( 'sgvx51_daily_acknowledgment_reminders', array( $this, 'send_daily_reminders' ) );
		if ( function_exists('as_next_scheduled_action') && !as_next_scheduled_action('sgvx51_daily_acknowledgment_reminders') ) {
			as_schedule_recurring_action( strtotime('09:00:00'), DAY_IN_SECONDS, 'sgvx51_daily_acknowledgment_reminders' );
		}
		
		// Register Module
		add_filter( 'sgvx51_get_module_rules', array( $this, 'get_instance' ) );
	}
	 ...
	 * Required by SGVX51_Module interface.
	 * 
	...
	public function handle_acknowledge_rule() {
		...
		// Save acknowledgment
		global $wpdb;
		$table = "{$wpdb->prefix}society_governx_rule_acknowledgments";
	...
	if ( $action === 'delete' && !empty($category_id) ) {
		...
		global $wpdb;
		$table = "{$wpdb->prefix}society_governx_rule_categories";
	...
	*/
	...
	public function render_page() {
		...
		SGVX51_Admin_App::render_view('rules', [
			'rules' => $rules,
