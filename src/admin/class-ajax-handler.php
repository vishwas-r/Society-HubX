...
class SHUBX51_AJAX_Handler {

	public function __construct() {
		add_action( 'wp_ajax_shubx51_get_module_config', array( $this, 'handle_get_module_config' ) );
		add_action( 'wp_ajax_shubx51_get_receipt', array( $this, 'handle_get_receipt' ) );
		add_action( 'wp_ajax_shubx51_approve_request', array( $this, 'handle_approve_request' ) );
		add_action( 'wp_ajax_shubx51_reject_request', array( $this, 'handle_reject_request' ) );
		add_action( 'wp_ajax_shubx51_bulk_process_requests', array( $this, 'handle_bulk_process_requests' ) );
		
		// Notifications
		add_action( 'wp_ajax_shubx51_get_channel_config', array( $this, 'handle_get_channel_config' ) );
		add_action( 'wp_ajax_shubx51_save_channel_config', array( $this, 'handle_save_channel_config' ) );
		add_action( 'wp_ajax_shubx51_toggle_channel', array( $this, 'handle_toggle_channel' ) );
		add_action( 'wp_ajax_shubx51_update_event_mapping', array( $this, 'handle_update_event_mapping' ) );
		add_action( 'wp_ajax_shubx51_get_template', array( $this, 'handle_get_template' ) );
		add_action( 'wp_ajax_shubx51_save_template', array( $this, 'handle_save_template' ) );
	}
	...
	private function get_module_config( $module ) {
		...
		switch ( $module ) {
			case 'residents':
				$config = array(
					'nonce'              => wp_create_nonce( 'shubx51_resident_nonce' ),
					'deleteNonce'        => wp_create_nonce( 'shubx51_delete_resident_nonce' ),
					'deleteHistoryNonce' => wp_create_nonce( 'shubx51_delete_history_nonce' ),
					'restoreNonce'       => wp_create_nonce( 'shubx51_restore_resident_nonce' ),
					'moveToHistoryNonce' => wp_create_nonce( 'shubx51_move_to_history_nonce' ),
				);
				break;

			case 'facilities':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_facility_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_delete_facility_nonce' ),
				);
				break;

			case 'notices':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_notice_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_delete_notice_nonce' ),
				);
				break;

			case 'documents':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_document_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_document_nonce' ),
				);
				break;

			case 'expenses':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_nonce' ),
				);
				break;

			case 'accounts':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_account_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_delete_invoice_nonce' ),
				);
				break;

			case 'vehicles':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_vehicle_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_delete_vehicle_nonce' ),
				);
				break;

			case 'flats':
                $config['nonce'] = wp_create_nonce('shubx51_add_flat_nonce');
                $config['deleteNonce'] = wp_create_nonce('shubx51_delete_flat_nonce');
                $config['hardDeleteNonce'] = wp_create_nonce('shubx51_hard_delete_flat_nonce');
                break;

			case 'staff':
				$config = array(
					'nonce'       => wp_create_nonce( 'shubx51_staff_nonce' ),
					'deleteNonce' => wp_create_nonce( 'shubx51_delete_staff_nonce' ),
				);
				break;
	...
	public function handle_get_receipt() {
		...
		$receipt_mgr = new SHUBX51_Receipt_Manager();
