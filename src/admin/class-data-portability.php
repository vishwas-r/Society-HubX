...
class SHUBX51_Data_Portability {

	public function __construct() {
		// Admin POST actions for Export/Import
		add_action( 'admin_post_shubx51_export_data', array( $this, 'handle_export_request' ) );
		add_action( 'admin_post_shubx51_import_data', array( $this, 'handle_import_request' ) );
	}
	...
	public function handle_export_request() {
		...
		check_admin_referer( 'shubx51_export_nonce' );

		// 1. Prepare Environment
		$upload_dir = wp_upload_dir();
		$temp_dir   = $upload_dir['basedir'] . '/SHUBX_temp_export_' . md5( uniqid() ) . '/';
	...
	public function handle_import_request() {
		...
		check_admin_referer( 'shubx51_import_nonce' );

		if ( empty( $_FILES['import_file']['tmp_name'] ) ) {
			wp_safe_redirect( admin_url( 'admin.php?page=shubx51-global-settings&tab=portability&error=no_file' ) );
			exit;
		}

		$target_table = isset( $_POST['target_table'] ) ? sanitize_text_field( wp_unslash( $_POST['target_table'] ) ) : '';
		$valid_tables = SHUBX51_DB_Router::TABLES;
		...
		wp_safe_redirect( admin_url( 'admin.php?page=shubx51-global-settings&tab=portability&imported=' . $count . '&errors=' . $errors ) );
		exit;
	}
