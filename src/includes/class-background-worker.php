...
class NAMMASOCIETY51_Background_Worker {
	...
	public function __construct() {
		...
		if ( $this->is_available() ) {
			add_action( 'nammasociety51_process_bulk_invoices', array( $this, 'process_bulk_invoices' ), 10, 4 );
			add_action( 'nammasociety51_process_notification_blast', array( $this, 'process_notification_blast' ), 10, 2 );
			add_action( 'nammasociety51_process_data_purge', array( $this, 'process_data_purge' ) );
		}

		// Daily Auto-Invoicing Cron Hook
		add_action( 'nammasociety51_daily_billing_check', array( $this, 'check_and_run_monthly_auto_invoicing' ) );
		if ( ! wp_next_scheduled( 'nammasociety51_daily_billing_check' ) ) {
			wp_schedule_event( time(), 'daily', 'nammasociety51_daily_billing_check' );
		}
	}
	...
	public function schedule_bulk_invoices( $month, $amount, $type = 'maintenance' ) {
		...
		as_enqueue_async_action( 'nammasociety51_process_bulk_invoices', array( 'month' => $month, 'amount' => $amount, 'type' => $type, 'offset' => 0 ) );
	}
	...
	public function process_bulk_invoices( $month, $amount, $type = 'maintenance', $offset = 0 ) {
		...
		foreach ( $residents as $r ) {
			...
			if ( ! $exists ) {
				...
				if ( $type === 'maintenance' && class_exists( 'NAMMASOCIETY51_Account_Manager' ) ) {
					$flat_obj = $db->get_row_by_field( 'flats', 'flat_number', $r['flat_no'] );
					$charge = NAMMASOCIETY51_Account_Manager::calculate_maintenance_charge( $flat_obj, $amount > 0 ? $amount : null );
		...
		if ( count( $residents ) === $limit ) {
            if ( $this->is_available() ) {
                as_enqueue_async_action( 'nammasociety51_process_bulk_invoices', array( 
                    'month'  => $month, 
                    'amount' => $amount, 
                    'type'   => $type, 
                    'offset' => $offset + $limit 
		...
		error_log( "NAMMASOCIETY51 Background Worker: Processed batch at offset $offset. Generated $generated_count $type invoices for $month." ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Operational/debug logging.
	}
	...
	public function process_notification_blast( $event_slug, $data ) {
		if ( ! class_exists( 'NAMMASOCIETY51_Plugin' ) ) return;
		...
		if ( isset( $nammasociety->notifications ) ) {
			...
			foreach ( $residents as $r ) {
				if ( ! empty( $r['wp_user_id'] ) ) {
					$nammasociety->notifications->trigger( $event_slug, $r['wp_user_id'], $data, true );
				}
			}
		}
	}
	...
	public function process_data_purge() {
		...
		$privacy = new NAMMASOCIETY51_Privacy_Manager();
	...
	public function check_and_run_monthly_auto_invoicing() {
		if ( get_option( 'nammasociety51_auto_invoicing_enabled', '0' ) !== '1' ) {
			return;
		}

		$current_month = current_time( 'Y-m' );
		$last_run = get_option( 'nammasociety51_last_auto_invoiced_month', '' );
		...
		$account_manager = new NAMMASOCIETY51_Account_Manager();
		$generated = $account_manager->perform_bulk_invoice_generation( $current_month, 0, 'maintenance' );

		update_option( 'nammasociety51_last_auto_invoiced_month', $current_month );
		error_log( "NAMMASOCIETY51 Auto-Invoicing: Generated $generated maintenance invoices for $current_month." ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Operational logging.
	}
}
