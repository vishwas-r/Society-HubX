...
class SGVX51_REST_Residents_Controller extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		// Apply DPDP masking
		$privileged = Society_GoVernX::get_instance()->rbac->check_capability( get_current_user_id(), 'view_pii' );
		
		foreach ( $residents as &$resident ) {
			if ( ! $privileged ) {
				$resident['phone'] = SGVX51_Privacy_Manager::mask_data( $resident['phone'] );
				$resident['email'] = SGVX51_Privacy_Manager::mask_data( $resident['email'] );
			}
		}
	...
	public function create_item( $request ) {
		...
		$resident_manager = new SGVX51_Resident_Manager();
