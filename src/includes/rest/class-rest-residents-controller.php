...
class SNESTX51_REST_Residents_Controller extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( $residents as &$resident ) {
			if ( ! $privileged ) {
				$resident['phone'] = SNESTX51_Privacy_Manager::mask_data( $resident['phone'] );
				$resident['email'] = SNESTX51_Privacy_Manager::mask_data( $resident['email'] );
			}
		}
	...
	public function get_item( $request ) {
		...
		if ( ! $resident ) {
			return new WP_Error( 'rest_resident_not_found', __( 'Resident not found.', 'society-nestx' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		$resident_manager = new SNESTX51_Resident_Manager();
	...
	public function create_item_permissions_check( $request ) {
		if ( ! Society_NestX::get_instance()->rbac->check_capability( get_current_user_id(), 'manage_residents' ) ) {
			return new WP_Error( 'rest_forbidden', __( 'You do not have permission to manage residents.', 'society-nestx' ), array( 'status' => 403 ) );
		}
		return true;
	}
}
