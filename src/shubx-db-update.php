...
// Load WordPress
$shubx51_wp_load = __DIR__ . '/../../wp-load.php';
if (!file_exists($shubx51_wp_load)) {
    // Try root search if not in standard wp-content/plugins/x location
    $shubx51_wp_load = dirname(__FILE__, 4) . '/wp-load.php';
}

if (!file_exists($shubx51_wp_load)) {
    die("Error: Could not find wp-load.php. Please ensure this script is inside the 'society-hubx/src/' directory on your WordPress installation.");
}

require_once $shubx51_wp_load;
...
echo "<h1>Society HubX - Database Update Utility</h1>";
...
update_option('shubx51_version', '1.0.2');
...
echo "<a href='" . esc_url( admin_url('admin.php?page=shubx51-settings') ) . "'>Return to Dashboard</a>";
